...
class statisticAction extends Action
{
	...
	{   
        $_REQUEST['queryRequest']=str_replace(' ', '%20', $_REQUEST['queryRequest']);
        set_time_limit(300);
	...
	public function createWeeklyLayout($epgids)
	{
		...
		foreach ($timerangeArray as $timekey => $timevalue) {
			...
			foreach ($epgcolumnArray as $key=>$epgcolumns)
			{
				...
				if(strcmp($lastepgcolumnstring,$epgcolumnstring) == 0){
					$endcol = $col;
					Log::write('begincol='.$begincol.'and endcol='.$endcol,Log::DEBUG);
				}else{
					if($begincol !=$endcol)
					{
						$objPHPExcel->getActiveSheet()->mergeCells($begincol.$row.':'.$endcol.$row);
					}
					Log::write('meet unmatch setcell value='.$epgcolumnstring,Log::DEBUG);
					$objPHPExcel->getActiveSheet()->getCell($begincol.$row)->setValue($epgcolumnstring);
					$lastepgcolumnstring = $epgcolumnstring;
					$begincol = $col;
					$endcol = $col;
				}
				if($key==6){
					Log::write('key=6 and endcol='.$endcol.'and begincol='.$begincol,Log::DEBUG);
					$objPHPExcel->getActiveSheet()->mergeCells($begincol.$row.':'.$endcol.$row);
					Log::write('setcell value='.$epgcolumnstring,Log::DEBUG);
					$objPHPExcel->getActiveSheet()->getCell($begincol.$row)->setValue($epgcolumnstring);
				}
			}
		}
